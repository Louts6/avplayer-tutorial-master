...
static const char* kAudioDecoderTag = "AudioDecoder";
namespace av {
...
void AudioDecoder::SetStream(AVStream* stream) {
    ...
    if (avcodec_parameters_to_context(m_codecContext, stream->codecpar) < 0) {
        LOGE(kAudioDecoderTag, "Failed to copy codec parameters to decoder context.");
        return;
    }

    const AVCodec* codec = avcodec_find_decoder(m_codecContext->codec_id);
    if (!codec) {
        LOGE(kAudioDecoderTag, "Codec not found.");
        return;
    }

    if (avcodec_open2(m_codecContext, codec, nullptr) < 0) {
        LOGE(kAudioDecoderTag, "Failed to open codec.");
        return;
    }
    ...
    if (!m_swrContext || swr_init(m_swrContext) < 0) {
        LOGE(kAudioDecoderTag, "Failed to initialize resampler.");
        return;
    }
...
void AudioDecoder::DecodeAVPacket() {
    ...
    if (packet->avPacket && avcodec_send_packet(m_codecContext, packet->avPacket) < 0) {
        LOGE(kAudioDecoderTag, "Error sending audio packet for decoding.");
        return;
    }

    AVFrame* frame = av_frame_alloc();
    if (!frame) {
        LOGE(kAudioDecoderTag, "Could not allocate audio frame.");
        return;
    }
    ...
    while (avcodec_receive_frame(m_codecContext, frame) >= 0) {
        ...
        if (ret < 0) {
            LOGE(kAudioDecoderTag, "Error while converting.");
            av_free(buffer);
            av_frame_free(&frame);
            return;
        }
